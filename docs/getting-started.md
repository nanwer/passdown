...
## First-run bootstrap and production status

The local setup above creates its own development owner and examples. Separately, the application has a first-run bootstrap for a configured, migrated database with no users: it creates an administrator and workspace and requires a password change before use.

These optional settings apply to that empty-database bootstrap, not to the owner account created by `pnpm local:setup`:

| Variable                       | Purpose                                                                        |
| ------------------------------ | ------------------------------------------------------------------------------ |
| `PASSDOWN_ADMIN_EMAIL`         | Administrator email; defaults to `admin@passdown.local`.                       |
| `PASSDOWN_ADMIN_PASSWORD`      | Initial password; generated when omitted.                                      |
| `PASSDOWN_ADMIN_PASSWORD_FILE` | Optional file for the generated password, written with owner-only permissions. |
| `PASSDOWN_WORKSPACE_NAME`      | Initial workspace name; defaults to `Workspace`.                               |

Bootstrap does not apply migrations, configure hosting, or provide backup and recovery. There is currently no supported production deployment recipe. Operational hardening, recovery procedures, and account recovery remain on the [roadmap](../ROADMAP.md).
