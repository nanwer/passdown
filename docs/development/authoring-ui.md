...
- **Preparation list**: keep the document’s item order and stable item keys while editing. Changing an item’s usage must not move it between parent groups or remount its controls. The add actions still distinguish reusable items from things used up or fitted; the reader can group them independently.
