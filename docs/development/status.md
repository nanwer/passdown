...
- A source-built Docker evaluation stack with private first-run settings, an explicit migration job, a non-root web image and a Caddy HTTPS proxy. Setup codes can be renewed only before setup completes. The operator command provides `help`, `version`, `status`, `migrate`, `setup-state` and `runtime-password`; its migration path shares the local runner and checks runtime-role safety. `upgrade.sh --build` upgrades a running installation: it backs up, builds, runs the new version's migrations while the old version keeps serving, and replaces web only if they succeed. See the [container evaluation guide](../self-hosting/development-stack.md). Published images remain upcoming.
