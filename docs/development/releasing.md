...
Releases are published only when the project owner says to publish. The [release workflow](../../.github/workflows/release.yml) builds the images, tests them, pushes them to GitHub Container Registry and drafts a GitHub release. It never publishes that release. Only exact version tags are pushed, never `latest` or another moving tag: an installation changes version only when its compose file names a new one, and an image already on a server is always the one its file names.
...
| **build**  | Builds `passdown` and `passdown-caddy` for linux/amd64 and linux/arm64, with provenance and an SBOM.                                                                                         | untagged images pushed by digest (`release` environment)              |
| **smoke**  | Renders the install files, then runs the deployment boot check (default login, Finish setting up, restart) from the rendered `compose.yaml` against the amd64 images.                        | nothing                                                               |
...
| **assets** | Renders `compose.yaml` with each image pinned as `:<version>@sha256:<digest>`, copies `upgrade.sh` and `backup.sh`, and writes `SHA256SUMS`.                                                 | a **draft** pre-release with those four files (`release` environment) |
...
3. Leave the two GHCR packages alone until the first release has been published (step 7 below).
...
6. Open the draft under **Releases**. Replace the placeholder notes with the reviewed release notes. Download the files, run `sha256sum --check SHA256SUMS`, and confirm every image in `compose.yaml` reads `ghcr.io/nanwer/…:<version>@sha256:…`. Paste that `compose.yaml` into a test Portainer (or run it with `docker compose up -d` in an empty folder), sign in with the default login and finish setting up. The owner then publishes the draft.
7. After the first release only: open each of the `passdown` and `passdown-caddy` packages under the owner's GitHub **Packages**. Set its visibility to **Public** and check that it is linked to the `nanwer/passdown` repository. Packages that a personal account creates start out private, so installs fail until this is done.
