...
Use a disposable installation from the section above, never your own data.

1. **Portainer / docker compose redeploy, failing migration.** Build a newer image with an extra migration (as `scripts/check-upgrade.sh` does), make that migration fail (for example create its table by hand), change the `x-passdown-image` line to the newer image and **Update the stack** (or `docker compose up -d`). Expect the redeploy to report `service "migrate" didn't complete successfully`, the `web` container to be in state **Created** with the new image and never started, the site to answer **502 Bad Gateway**, and `ops status` from the old image to show the migration count unchanged.
2. **Going back.** Put the previous image back on the line and redeploy. Expect the site to answer again with your content and account.
3. **upgrade.sh, failing migration.** With the failure still in place, change the image line again and run `sh upgrade.sh --project <name>` beside `compose.yaml`. Expect a backup under `backups/`, exit 1 and **Migrations failed; the site is still running the previous version.** While it runs and afterwards, the site keeps working; `docker compose ps` shows the same web container; `upgrade.log` records the attempt.
4. **upgrade.sh, success.** Remove the cause and run it again. Expect **Upgrade complete.**, the new version from `ops version`, one more migration in `ops status`, and your content intact.
5. **Refusal.** `docker compose stop web`, then `sh upgrade.sh`: expect a refusal telling you to use `docker compose up -d`, and no backup.
6. **Automated rehearsal:** with `passdown:local` and `passdown-caddy:local` built, run `sh scripts/check-upgrade.sh` from the repository root. Expect it to print the observed redeploy behaviour (`old web container kept: no`, `web container after redeploy: created`, `health through the proxy: 502`, `migrations recorded: N (before: N)`, then `after putting the previous image back: health 200` and `upgrade.sh with the failing migration: old web kept, health 200`) and **Upgrade check passed**, with no leftover `passdown-check-upgrade-*` containers or volumes.
