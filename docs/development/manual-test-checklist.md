...
Testing a server installation instead? Follow the [self-hosting guide](../self-hosting/install.md) on a separate installation, then use the sections on installation, first sign-in, backups, restore, upgrades, account recovery and the release workflow below.

## Install from the compose file, default login and Finish setting up

Use a disposable project on a machine with Docker, never your own data. Until images are published, build them first from the repository root: `cd deploy && PASSDOWN_BUILD_TAG=local docker compose -f compose.yaml -f compose.build.yaml build`, then use a copy of `compose.yaml` whose `x-passdown-image` line reads `passdown:local` and whose proxy `image:` reads `passdown-caddy:local`.

1. **Paste and deploy.** In Portainer, **Stacks → Add stack → Web editor**, paste that file, add the environment variable `PASSDOWN_URL=https://<this machine's IP>:8443`, and deploy. Without Portainer: save it as `compose.yaml` in an empty folder, `export PASSDOWN_URL=https://<IP>:8443`, then `docker compose -p passdown-manual up -d`. Expect no request for a settings file or project name, `init` and `migrate` exited 0, `postgres` and `web` healthy, and only the proxy publishing a port: `8443->443`. Nothing listens on 80 or 443 (`docker compose ps`).
2. **Certificate.** Open `https://<IP>:8443`. Expect one certificate warning; after accepting it, the front page redirects to **Sign in**. Open `https://localhost:8443` on the same machine as well: the page loads (same certificate, issued to `localhost`).
3. **Sign-in hint.** On **Sign in**, expect a blue notice: **First time? Sign in with admin@example.com and changeme.** Check light and dark themes and a 390px-wide window.
4. **Another address.** At `https://localhost:8443` (not `PASSDOWN_URL`), expect a red notice **Passdown is set up for https://<IP>:8443. Open it at that address, or ask the operator to set PASSDOWN_URL to https://localhost:8443.** Signing in there shows the same message.
5. **Default login.** At `PASSDOWN_URL`, sign in with `admin@example.com` / `changeme` using only the keyboard (Tab, Enter). Expect **Finish setting up Passdown**, focus in **Your name**, and in the header only the theme toggle and **Sign out**, no Studio or Administration tabs.
6. **Nothing else is reachable.** Open `/studio`, `/account` and `/admin/accounts`: each shows the same form. `/api/studio/session` answers 403 `SETUP_REQUIRED`. `docker compose logs web` contains `"event":"setup.default-login"`, and `/api/health` answers `setup-required`.
7. **Refusals keep your input.** Enter a name, `admin@example.com`, a 12-character password, a different confirmation and a workspace name, then press Enter. Expect focus on **Your email address** with **Use your own email address, not admin@example.com.** Fix it: focus moves to **New password again** with **The passwords do not match.** A password under 12 characters says **Use at least 12 characters.** Typed values stay.
8. **Finish.** Correct the fields and choose **Finish setting up**. Expect the workspace to open (`/studio/<workspace>`), the header to show your name, **Administration** to be present, and `/api/health` to answer `ready`.
9. **The default login is gone.** Sign out. **Sign in** no longer shows the hint; `admin@example.com` / `changeme` is refused. Your own address and password work. `docker compose run --rm -T ops setup-state` prints `complete`; after `docker compose restart web` the log has no new `setup.default-login` line.
10. **Secrets.** `docker compose exec postgres stat -c '%a %u %g' /run/passdown/owner/database-owner-password` prints `440 10001 70`; `docker compose exec web stat -c '%a %u %g' /run/passdown/app/session-secret` prints `400 10001 10001`; `docker compose exec web ls /run/passdown/owner` is empty; `docker inspect` of web shows no variable containing `OWNER`. `docker compose run --rm init` prints **Secrets already exist; nothing was changed.**
11. **Restart keeps everything.** `docker compose down` (no `-v`), then `up -d` again: sign in with your own account, content remains.
12. **Automated:** `sh scripts/deployment-boot-check.sh` (or `PASSDOWN_IMAGE=… PASSDOWN_PROXY_IMAGE=…` for existing images) and `pnpm test:setup` pass.

Remove the test project with `docker compose -p passdown-manual down -v` (or delete the stack and its volumes in Portainer) when finished.
...
Use a disposable installation from the section above, never your own data. Build a newer image with two extra migrations, as `scripts/check-upgrade.sh` does, and tag the proxy image under a second name so the proxy line changes too, as in a real release.

1. **Starting page.** On the running installation, `docker compose stop web`, then open any page. Expect **503** and **This site will be back in a moment**, reloading every 10 seconds, in light and dark themes and at 390px wide. `curl -k https://localhost:8443/api/health` answers `503` `{ "status": "starting" }` with `Retry-After: 10`. `docker compose start web` brings the site back.
2. **Redeploy, failing migration.** Make the first new migration fail (create its table by hand), change both the `x-passdown-image` line and the proxy's `image:` line to the newer images, and **Update the stack** (or `docker compose up -d`). Expect the redeploy to report `service "migrate" didn't complete successfully: exit 1`; `docker compose ps --all` to show `migrate` **Exited (1)**, `web` **Created** (never started) and `proxy` running with the new image; and every page to answer **503** with **This site is offline while its upgrade is sorted out**, **No data was changed** and how to put the previous version back. `/api/health` answers `{ "status": "upgrade-failed", "databaseChanged": false }`; `docker compose logs migrate` ends with **Passdown did not start this version, and no data was changed.** `ops status` from the old image shows the migration count unchanged.
3. **Going back.** Put the previous versions back on both lines and redeploy. Expect the site to answer again with your content and account, and `docker compose exec proxy ls -A /srv/passdown-status` to print nothing.
4. **upgrade.sh, failing migration.** With the failure still in place, change the image lines again and run `sh upgrade.sh --project <name>` beside `compose.yaml`. Expect a backup under `backups/`, exit 1 and **Migrations failed; the site is still running the previous version.** While it runs and afterwards, the site keeps working; `docker compose ps` shows the same web container; `upgrade.log` records the attempt; the status volume stays empty.
5. **Redeploy, second migration fails.** Drop the first table, create the second migration's table by hand, and redeploy the newer versions. Expect the page to say **Some of the new version's database changes were applied** (and not "No data was changed"), health to answer `{ "status": "upgrade-incomplete", "databaseChanged": true }`, and one more migration in `ops status`.
6. **Redeploy after fixing the cause.** Drop the second table and redeploy. Expect the site back on the new version with your content, both migrations in `ops status`, and the status volume empty.
7. **upgrade.sh, success.** Change the image line to a further version (the same build under another tag will do) and run `sh upgrade.sh`. Expect **Upgrade complete.**, a new backup, the new version from `ops version`, and your content intact.
8. **Refusal.** `docker compose stop web`, then `sh upgrade.sh`: expect a refusal telling you to use `docker compose up -d`, and no backup.
9. **Automated rehearsal:** with images built from this checkout, run `PASSDOWN_IMAGE=… PASSDOWN_PROXY_IMAGE=… sh scripts/check-upgrade.sh` from the repository root (defaults: `passdown:local`, `passdown-caddy:local`). Expect it to print the observed redeploy behaviour (`redeploy exit: failed`, `web container after redeploy: created …`, `proxy after redeploy: running …`, `health through the proxy: 503 { "status": "upgrade-failed", "databaseChanged": false }`, `after putting the previous versions back: health 200`, `upgrade.sh with the failing migration: old web kept, health 200`, `partly applied redeploy: health 503 { "status": "upgrade-incomplete", … }`, `successful redeploy: health 200 …`) and **Upgrade check passed**, with no leftover `passdown-check-upgrade-*` containers or volumes. `node scripts/check-proxy.mjs --live` covers the three notices without a full stack.
...
1. Follow **Install Passdown on your own server** from an empty folder, using [Build from source](../self-hosting/install.md#build-from-source) until images are published. Every command should work as written, `docker compose ps --all` should match the described states, and health should report `setup-required` before finishing setup and `ready` after.
2. Compare the guide's **The compose file** section with `deploy/compose.yaml`: identical (a test enforces it). In the configuration reference, check that every variable in the compose file is described.
3. Set `PASSDOWN_SOURCE_URL=https://example.org/fork` (stack variable or `.env`) and redeploy. Expect every **Source code** link to point there. Remove it and redeploy: the links return to the Passdown commit.
4. Follow **Using your own proxy (Nginx Proxy Manager)** if you have one: forward `https` to `server:8443`, set `PASSDOWN_URL` to the public address and redeploy. Sign-in works at the public address with a trusted certificate.
5. Pick three troubleshooting entries (for example signing in at another address, the default login after setup, port 8443 taken) and reproduce them. The description should match what you see, and its fix should work.
6. Run `pnpm lint`. It includes the configuration check: temporarily delete one variable's row from the configuration reference and expect lint to name it; restore the row.
...
2. Run `docker compose logs web | grep '"event":"startup"'`. Expect one line per start with the version, mode `production`, the origin, `schema`, `media` and `setup` (`default-login` before finishing setup, `complete` after), and no secrets.
...
1. Save a draft with a picture. Beside the installation's `compose.yaml`, run `docker compose run --rm -T ops verify-media --checksums --json`. Expect no missing, damaged or dangling pictures and exit 0. The command must not change the draft or files.
2. Run `sh backup.sh /path/to/private-backups` with a real writable private directory (for a Portainer stack: `sh backup.sh --file compose.yaml --project <stack> /path/to/private-backups`). Expect a final archive path only after the archive passes its offline check. On macOS, run `stat -f '%Lp' <archive>` (Linux: `stat -c '%a' <archive>`): expect `600`.
...
1. On the source, publish a guide with a picture, save a private guide, and create a pending invitation. Make a backup. In a new folder with the same version's `compose.yaml` and a `.env` naming another `PASSDOWN_URL` and `PASSDOWN_PORT`, run the offline check without starting anything. Expect a compatibility success message and no database changes.
2. Run `docker compose run --rm -T ops restore < /path/to/private-backups/passdown-example.tar` in that folder. Expect verification progress, a snapshot-dated access report, cancellation of pending invitations, and **Restore ready**. No web service starts during restore. Afterwards, run `docker compose run --rm --no-deps ops sh -c 'find "$GUIDE_MEDIA_ROOT" -name database.dump'` and expect no output: the received archive was unpacked in the operator container's temporary space and deleted once the database loaded, never written to the shared picture volume.
3. Start the target with `docker compose up -d`. Sign in with the backed-up manager account. Expect the guides and their original pictures. Signed out, public guides load and private guides remain inaccessible. The old pending invitation no longer works; the sign-in page doesn't offer the default login and `admin@example.com` / `changeme` is refused.
4. Run `docker compose run --rm -T ops verify-media --checksums --json`. Expect zero missing, damaged or dangling files for a complete backup. Compare a guide's text and picture to the untouched source.
...
No restore screen is added to Studio. Published images and the remaining release checks are unfinished. Archive checks alone are not proof of disaster recovery.
...
3. **Separate projects:** two copies of `compose.yaml` started with different `-p` names (or two Portainer stacks) get separate volumes: `docker volume ls` shows `<name>_database` for each, and each gets its own default login.
...
Use a disposable installation (the compose file above) or the setup browser suite's database. Do not clear an existing installation to try this. Local sample accounts created by `pnpm local:setup` never get the default login.

| Feature                     | Exact steps                                                                                                     | Expected result                                                                                                                                                 |
| --------------------------- | --------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Fresh installation          | Deploy the compose file; open `/`.                                                                              | Redirect to `/studio`, then **Sign in** with **First time? Sign in with admin@example.com and changeme.** Health: `setup-required`.                             |
| Default login only          | Sign in as `admin@example.com` / `changeme`; open `/studio`, `/account`, `/admin/accounts`, `/studio/anything`. | Only **Finish setting up Passdown** and **Sign out**. API routes answer 403 `SETUP_REQUIRED`; `POST /api/studio/password` answers 422 naming finish setting up. |
| Validation                  | Submit with `admin@example.com`, then mismatched passwords, then an 11-character password.                      | Inline messages under the field, focus on it, typed values kept, nothing sent to the server for these.                                                          |
| Finish                      | Enter name, `Owner@Example.org`, a matching 12–200-character password and a workspace name; submit once.        | The workspace opens; the account is the installation administrator; health `ready`.                                                                             |
| Default login retired       | Sign out; sign in with `admin@example.com` / `changeme`; then with `owner@example.org` and the chosen password. | The default is refused and the hint is gone; your account works with either letter case.                                                                        |
| Old setup address           | Open `/setup`.                                                                                                  | Redirects to the studio (or sign-in when signed out).                                                                                                           |
| Existing or restored data   | Run `ops migrate` against an installation that already has accounts (or after a restore).                       | `0 applied now` and no default login created; `ops setup-state` stays `complete`.                                                                               |
| Account-less database       | A database with a workspace but no accounts.                                                                    | Every page: **Passdown has no account yet**; `ops setup-state` prints `no-account`; the log has `setup.no-account`.                                             |
| Keyboard and small screens  | Sign in and finish with the keyboard only, at 320px and 390px, in light and dark themes.                        | Labels, focus, errors and the button remain visible; no sideways scrolling.                                                                                     |
| Production without settings | Start a production build without database/sign-in settings. Open `/` and request `/api/health`.                 | Installation page and 503 `not-configured`.                                                                                                                     |
| Production preview guard    | Set `GUIDE_DEMO_PREVIEW=1` on a configured production build and open the preview path.                          | No preview identities.                                                                                                                                          |

Automated: `pnpm test:setup` runs the first-login journey in Chromium, Firefox and WebKit sequentially, each with a new disposable database (which gets the default login as `passdown migrate` would) and a server on port 3106. Set `SETUP_EVIDENCE_DIR=<folder>` to save screenshots of the sign-in hint and the Finish setting up form in light and dark at 390 and 1280 pixels. Traces are off so entered credentials are never recorded.
...
Use the [container evaluation guide](../self-hosting/development-stack.md) and a separate project; never reset an existing database to test these flows.

1. From `deploy`, run `export PASSDOWN_URL=https://localhost:18443 PASSDOWN_PORT=18443` and `docker compose -p passdown-evaluation -f compose.yaml -f compose.build.yaml up -d --build`. Expect no settings file to be needed.
2. Sign in with the default login and finish setting up (see the first section). Run `docker compose -p passdown-evaluation -f compose.yaml -f compose.build.yaml run --rm -T ops setup-state`: expect `complete`. Run `ops migrate`: expect no new migrations and no default login. Run `ops help`: expect the supported commands, including `init-secrets`. An unknown command returns exit 2; a missing secret file returns exit 3 naming the `*_FILE` variable without printing a secret.
   Run `ops version`: expect `passdown <version> (revision <commit>)`. Run `ops status`: expect `Database schema is current (N migrations applied).` and `Setup: complete`.
3. Create a guide, upload a photo, save it, stop with `docker compose … down` (no `-v`) and start again. Expect the guide and photo to remain. Open the private photo signed out: expect no access.
4. Run `node scripts/check-proxy.mjs --live` and `PASSDOWN_SKIP_BUILD=1 sh scripts/deployment-boot-check.sh` from the repository root. Expect both to pass and remove their own disposable Docker resources. They must not touch the local development stack.

No published images are delivered yet. Public-domain clean-host and physical-browser release checks remain outstanding.
...
1. **Locally, from the repository root:** render install files with stand-in digests.
   ...
   Expect four files: `compose.yaml`, `upgrade.sh`, `backup.sh` and `SHA256SUMS`. The `x-passdown-image` line and the proxy's `image:` read `ghcr.io/nanwer/<image>:0.1.0-alpha.1@sha256:…`, and postgres keeps its digest. There is no `build:` anywhere. From that directory, `sha256sum --check SHA256SUMS` (or `shasum -a 256 -c SHA256SUMS`) reports every file `OK`. Change one character in `compose.yaml` and run `node scripts/render-release-assets.mjs --check /tmp/passdown-assets --version 0.1.0-alpha.1`. Expect exit 1 naming `compose.yaml`. The script refuses an output directory that already holds files.

2. **Locally, the boot check from rendered files:** build the two images as in CI, render them into an empty directory using `docker image inspect --format '{{.Id}}' <image>` as each digest, then run `PASSDOWN_DEPLOY_DIR=<that directory> PASSDOWN_IMAGE=passdown:local PASSDOWN_PROXY_IMAGE=passdown-caddy:local sh scripts/deployment-boot-check.sh`. It ends with **Deployment boot check passed** and removes its disposable project.

3. **On GitHub (maintainer):** open **Actions → Release → Run workflow** on `main`, leaving **dry_run** ticked. Expect **Check the version**, four **Build** jobs (two images × amd64/arm64) and **Smoke test from the install file (amd64)** to pass without asking for an environment approval. **Tag the release images** and **Draft the GitHub release** are skipped. Afterwards no new package version appears under the owner's GitHub Packages, no release or tag is created, and the run has no artifacts.
