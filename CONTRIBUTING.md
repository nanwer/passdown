...
Leave the test ports available and run one instance of each browser or database suite at a time. Browser suites start and own their test server; an occupied port fails the run instead of reusing another run's server, which could disappear during teardown. The fixture suite defaults to two workers so the development server and browser interactions share a predictable amount of CPU. Before the first test, each browser suite requests every page and route handler once (`tests/support/warm-routes.ts`), so no assertion's time limit includes the development server's first-request compile. Hosted runners are roughly four times slower than a recent laptop; to reproduce a hosted timing failure locally, run the test against a fresh server with the assertion timeout scaled down accordingly. Run the production build and browser suites sequentially when checking timing-sensitive interactions. Use these databases only for tests. Set `PLAYWRIGHT_CHANNEL=chrome` to run browser tests with an installed Google Chrome. Do not point test runners at a database containing content you want to keep.
